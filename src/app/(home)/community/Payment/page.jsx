  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      if (user?.email) {
        axios
          .get(
            `https://weather-cast-server.vercel.app/payment/success/${user.email}?paidStatus=true`
          )
          .then((successResponse) => {
            ...
            axios
              .get(
                `https://weather-cast-server.vercel.app/payment/fail/${user.email}?paidStatus=false`
              )
              .then((failResponse) => {
                console.log(failResponse.data);

                const combinedData = [
                  ...successResponse.data.filter(
                    (payment) => payment.paidStatus === true
                  ),
                  ...failResponse.data.filter(
                    (payment) => payment.paidStatus === false
                  ),
                ].sort(
          ...
          .catch((successError) => {
            console.error(
              "Error fetching successful payment data:",
              successError
            );
          });
      }
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4 ">
      <h1 className="text-2xl font-semibold mb-4 text-center">
        Successful Payments: {countSuccessfulPayments()}
      </h1>
